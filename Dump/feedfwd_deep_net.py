...
class FFN_architecture:
    def __init__(self):
        ...
        with tf.variable_scope("FFN", reuse=True) as scope:
             self.o = self.mlpnet(self.x)
             scope.reuse_variables()
             self.o_test = self.mlpnet(self.x_test)
    ...
    def mlpnet(self, x,_dropout= 0.7, nn1= 11, nn2=5, nn3=5,num_att=14):
...
def train_neural_network(att_data, clipped_spikes, hm_epochs=2, batch_size= 100):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(hm_epochs):
            ...
            spike_test= model.o_test.eval({model.x_test:test_x})
