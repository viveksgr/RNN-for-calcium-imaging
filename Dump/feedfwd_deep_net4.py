...
class FFN_architecture:
    def __init__(self):
        ...
        self.y = tf.placeholder(tf.int32,shape=(self.batch_size,1)) # Labels
        self.o = self.mlpnet(self.x) # Predictions
#        self.loss = tf.reduce_sum(tf.reduce_mean(tf.abs(tf.sub(self.o, self.y)), 0) # Custom L1 loss function
        self.loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(self.o,self.y))
...
def train_neural_network(att_data, clipped_spikes, hm_epochs=4):    
